...
def preprocess_and_find_similarities(input_title, news_dict, word_scores, similarity_threshold):
    all_titles = []
    sites = []
    ...
    score = sum(word_scores.get(word, 0) for word in lemmatized_words)
...
# Streamlit app
st.title('News Similarity Analysis')

uploaded_file = st.file_uploader("Choose an Excel file", type="xlsx")

if uploaded_file:
    news_dict = create_news_dict(uploaded_file)
    input_title = st.text_input("Enter a news title:")
    similarity_threshold = st.slider("Similarity Threshold", 0.0, 1.0, 0.75)

    if st.button("Analyze"):
        if input_title:
            results = preprocess_and_find_similarities(input_title, news_dict, word_scores, similarity_threshold)

            st.write(f"**Title Analysis for: {input_title}**")
            st.write(f"Score: {results['score']}")

            st.write("**Analysis of Similar Titles**")
            
            for site, title, score in results['similar_titles']:
                st.markdown(f"**Site:** {site}  \n**Title:** {title}  \n**Similarity:** {score * 100:.1f}%")

            st.write(f"Total Similarity Score: {results['similarity_score']}")
