...
def score_1gram_words(lemmatized_words, word_scores):
    ...
    for word in lemmatized_words:
        ...
        total_score += score
    st.write(f"Score of one gram words : {total_score}")
    return total_score
...
def score_2gram_words(lemmatized_words, two_gram_word_scores):
    ...
    for ngram in feature_names:
        ...
        total_score += score
    st.write(f"Score of two gram words : {total_score}")
    return total_score
...
if uploaded_file:
    ...
    if st.button("Analyze"):
        if input_title:
            ...
            st.write(f"Total Title Corroboration/Credibility Score: {results['similarity_score']}")
            st.write("-----------------------------------------------------------------------------")
            st.write("Final score : one gram word scores + two gram word scores + corroboration/credibility score")
            st.write(f"Final score : {results['one_gram_score']} + {results['two_gram_score']} + {results['similarity_score']}")
