...
# Initialize VADER, AFINN, and DistilBERT
...
classifier = pipeline('sentiment-analysis', model=distilbert_model, tokenizer=tokenizer)
...
def combined_sentiment(text):
    ...
    distilbert_score = distilbert_sentiment(text)
    combined_score = (vader_score + afinn_score + distilbert_score) / 3
...
def preprocess_and_find_similarities(input_title, news_dict, word_scores, two_gram_word_scores, similarity_threshold):
    ...
    sentiment_score = combined_sentiment(input_title)
    ...
    result = {
        ...
        "final_score": total_score + similarity_score + abs(sentiment_score*5),
...
if uploaded_file:
    ...
    if st.button(":orange[Analyze]"):
        if input_title:
            ...
            st.subheader(":orange[Sentiment scoring]" ,divider="orange")
            st.write(f"**:orange[Sentiment Score [-4 to 4]]**: {results['sentiment_score']:.1f}")
            st.write(f"**:orange[Scaled Sentiment Score [-20 to 20]]**: {results['sentiment_score']*5:.1f}")
            st.write("-----------------------------------------------------------------------------")

            st.subheader(":orange[Final Score of Title]" ,divider="orange")
            st.write("Final score : one gram word scores + two gram word scores + corroboration/credibility score + |sentiment score|")
            st.write(f"Final score : :orange[{results['one_gram_score']}] + :orange[{results['two_gram_score']}] + :orange[{results['similarity_score']}] + :orange[{abs(results['sentiment_score']*5):.1f}]")
